# Rite of Renaissance™

# Rite of Renaissance™

[![Platform DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.17172637.svg)](https://doi.org/10.5281/zenodo.17172637)
[![Study DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.17396753.svg)](https://doi.org/10.5281/zenodo.17396753)
[![License](https://img.shields.io/badge/License-MIT-green.svg)](LICENSE)
[![Status](https://img.shields.io/badge/Status-Active-brightgreen)](https://riteofrenaissance.github.io/riteofrenaissance/)

**A philosophical framework and practical toolkit for conscious existence in the age of uncertainty.**

---

## 🌟 Overview

Rite of Renaissance is an experimental project exploring the intersection of:
- **Existential philosophy** (meaning-making, ambiguity tolerance)
- **Contemplative practice** (structured introspection, symbolic rituals)
- **Digital tools** (interactive simulations, guided protocols)

The project offers accessible protocols for individuals seeking to develop resilience in the face of uncertainty, complexity, and the unknown.

---

## 🎯 Core Components

### 1. **Safe Aperture Simulation**
An interactive web-based training program for developing ambiguity tolerance through graduated exposure.

**🔗 Access:** [Safe Aperture Platform](https://riteofrenaissance.github.io/riteofrenaissance/safe_aperture_simulation.html)

**Features:**
- 5 progressive levels (everyday → existential ambiguity)
- Built-in safety mechanisms (Emergency Exit, grounding techniques)
- Self-paced, accessible anywhere
- No login required

**Use Cases:**
- Personal development
- Anxiety management
- Decision-making under uncertainty
- Philosophical inquiry

---

### 2. **Philosophical Framework**
Conceptual foundations drawn from:
- Phenomenology (Heidegger, Merleau-Ponty)
- Existentialism (Sartre, Camus)
- Pragmatism (James, Dewey)
- Contemplative traditions (Buddhist mindfulness, Stoic practice)

**Key Concepts:**
- **Ambiguity as generative** (not merely problematic)
- **Understanding as practice** (not possession)
- **Meaning as emergent** (not pre-given)

---

### 3. **Ritual Protocols**
Structured practices for engaging with uncertainty:
- Sovereign Initiation (self-determination ritual)
- Meaning Cartography (personal value mapping)
- Existential Inquiry (guided questioning)

---

## 📚 Documentation

### Quick Start
1. Visit the [Safe Aperture Simulation](https://riteofrenaissance.github.io/riteofrenaissance/safe_aperture_simulation.html)
2. Start with Level 1 (10 minutes)
3. Progress at your own pace
4. Use Emergency Exit if needed

### Repository Structure
```
riteofrenaissance/
├── index.html                          # Main landing page
├── safe_aperture_simulation.html       # Interactive training platform
├── docs/
│   ├── philosophy.md                   # Theoretical foundations
│   ├── protocols.md                    # Ritual instructions
│   └── faq.md                          # Common questions
├── assets/
│   ├── css/                            # Styling
│   ├── js/                             # Interactive elements
│   └── images/                         # Visual resources
└── research/
    └── → See safe-aperture-study repo  # Scientific validation
```

---

## 🔬 Scientific Validation

Safe Aperture is currently undergoing rigorous empirical testing.

**📊 Ongoing Study:**
- **Type:** Pre-registered Randomized Controlled Trial
- **Sample:** N=30 (intervention vs. waitlist control)
- **Timeline:** December 2025 - February 2026
- **Status:** IRB submission in progress

**📁 Study Repository:**
[riteofrenaissance/safe-aperture-study](https://github.com/riteofrenaissance/safe-aperture-study)

**📖 Study DOI:**
[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.17396753.svg)](https://doi.org/10.5281/zenodo.17396753)

**Complete Protocol:**
- [Study Protocol](https://github.com/riteofrenaissance/safe-aperture-study/blob/main/PROTOCOL.md)
- [Pre-registration](https://github.com/riteofrenaissance/safe-aperture-study/blob/main/PRE-REGISTRATION.md)
- [Measures](https://github.com/riteofrenaissance/safe-aperture-study/tree/main/measures)

---

## 🎨 Design Philosophy

### Accessibility
- No account creation required
- Works on mobile and desktop
- Free and open-source
- Multilingual support (planned)

### Safety
- Built-in distress management
- Emergency exit always available
- Grounding techniques included
- Clear content warnings

### Transparency
- Open-source codebase
- Pre-registered research
- Public documentation
- Community feedback encouraged

---

## 🌐 Use Cases

### For Individuals
- Develop comfort with uncertainty
- Improve decision-making under ambiguity
- Reduce anxiety around the unknown
- Cultivate philosophical resilience

### For Researchers
- Ambiguity tolerance training protocol
- Intervention for intolerance of uncertainty
- Validated platform for studies
- Open materials for replication

### For Educators
- Teaching tool for philosophy courses
- Experiential learning component
- Discussion catalyst for existential themes

### For Therapists
- Supplementary tool for anxiety treatment
- Homework between sessions
- Client self-exploration resource
- *(Not a substitute for professional care)*

---

## 📖 Citation

### Citing the Platform

Rite of Renaissance. (2025). Rite of Renaissance: Safe Aperture and 
Philosophical Protocols (Version 1.0.0). Zenodo. 
https://doi.org/10.5281/zenodo.17172637


### Citing the Research Study

Rite of Renaissance. (2025). Safe Aperture Micro-Pilot Study: 
Pre-registered Protocol (Version 1.0-preregistration). Zenodo. 
https://doi.org/10.5281/zenodo.17396753

---

## 🤝 Contributing

We welcome contributions:
- **Code:** Submit pull requests for platform improvements
- **Content:** Suggest new levels, exercises, or protocols
- **Translation:** Help make this accessible in other languages
- **Research:** Collaborate on validation studies
- **Feedback:** Report bugs or user experience issues

**Contact:** riteofrenaissance@gmail.com

---

## ⚠️ Disclaimer

**Not Medical Advice:**  
This platform is for educational and personal development purposes. It is not a substitute for professional mental health treatment. If experiencing severe distress, please contact:
- Emergency services: 911 (US) or local equivalent
- Crisis hotline: 988 (US Suicide & Crisis Lifeline)
- Mental health professional

**Research Participation:**  
If interested in participating in our research study, see [safe-aperture-study](https://github.com/riteofrenaissance/safe-aperture-study) for eligibility and details.

---

## 📜 License

This project is licensed under the [MIT License](LICENSE).

**You are free to:**
- Use commercially or non-commercially
- Modify and adapt
- Distribute
- Use in research (with citation)

**Attribution required:** Cite as shown above.

---

## 🔗 Links

- **Live Platform:** [https://riteofrenaissance.github.io/riteofrenaissance/](https://riteofrenaissance.github.io/riteofrenaissance/)
- **Safe Aperture:** [Simulation Link](https://riteofrenaissance.github.io/riteofrenaissance/safe_aperture_simulation.html)
- **Research Study:** [GitHub Repo](https://github.com/riteofrenaissance/safe-aperture-study)
- **DOI:** [10.5281/zenodo.17172637](https://doi.org/10.5281/zenodo.17172637)
- **Contact:** riteofrenaissance@gmail.com

---

## 🌱 Roadmap

### Current (Q4 2025)
- ✅ Safe Aperture Simulation (Levels 1-5)
- ✅ Web platform launch
- 🔄 Micro-pilot research study (in progress)

### Upcoming (Q1 2026)
- [ ] Mobile app version
- [ ] Community forum
- [ ] Additional ritual protocols
- [ ] Research results publication

### Future (2026+)
- [ ] AI-assisted personalization
- [ ] Multilingual support (Arabic, French, Spanish)
- [ ] Integration with wearables (biofeedback)
- [ ] Therapist/educator certification program

---

## 💬 Community

Join the conversation:
- **GitHub Discussions:** [Link when enabled]
- **Email Updates:** riteofrenaissance@gmail.com
- **Research Participation:** See [study repo](https://github.com/riteofrenaissance/safe-aperture-study)

---

## 🙏 Acknowledgments

This project draws inspiration from:
- Existential philosophy and phenomenology
- Acceptance and Commitment Therapy (ACT)
- Mindfulness-based interventions
- Open science movement
- Creative commons community

Special thanks to all beta testers and early adopters.

---

**Last Updated:** October 20, 2025  
**Version:** 1.0.0  
**Maintained by:** Rite of Renaissance Team
## 🌐 Deployment

- **Netlify**: riteofrenaissance.netlify.app
- **GitHub**: github.com/riteofrenaissance

## 🔧 Technologies

### **Current Production Stack**
- **Frontend:** HTML5, CSS3, JavaScript (Vanilla)
- **Deployment:** Netlify, GitHub Pages
- **Version Control:** Git & GitHub
- **Design:** Responsive Design, Mobile-First, WCAG 2.1

### **Development & Tools**
- **Editor:** VS Code
- **Design:** Figma (for UI/UX design)
- **Analytics:** Google Analytics (planned)
- **Testing:** BrowserStack (for cross-browser testing)

### **Future Expansion Ready**
- **Backend:** Python/Flask, Google Gemini AI
- **Infrastructure:** Docker, Nginx, Gunicorn
- **Database:** PostgreSQL, MongoDB
- **Real-time:** WebSockets (for interactive features)

### **Methodologies**
- **Progressive Web App** principles
- **API-First Design**
- **Microservices Ready** (for scaling)
- **CI/CD Pipeline** (planned)
  
---

**Developed for Rite of Renaissance**
